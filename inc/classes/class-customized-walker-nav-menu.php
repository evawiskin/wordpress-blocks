...
class Customized_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	function start_el(&$output, $menu_item, $depth = 0, $args = array(), $id = 0) {
		...
		// If using megamenu template, skip level 1 items (output of child items handled manually in megamenu template)
		if ($this->level_template === "nav-megamenu" && $depth === 1) {
			return;
		}
		...
		ob_start();
		get_template_part("template-parts/nav/template-part", "nav-item", ["depth" => $depth, "menu_item" => $menu_item]);
	...
	public function start_lvl(&$output, $depth = 1, $args = null) {
		...
		ob_start();
		get_template_part("template-parts/nav/template-part", "nav-level", ["depth" => $depth]);
		$output .= ob_get_contents();
		ob_end_clean();
